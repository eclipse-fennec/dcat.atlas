...
| FR-16 | Public URLs unaltered | ✅ | Stored verbatim; foreign IRIs — publishers, licences, vocabulary terms — pass through untouched, recognised structurally by not sitting under one of our bases. The endpoint-awareness configuration this row used to say was missing is `PublicIris` (see FR-18). |
| FR-17 | Reachability check (opt.) | ⬜ | Split out of N16, whose other two clauses are done. Shape still undecided — FR-17 says "on request, the operation checks" (per-write) while N16 said "health check" (periodic sweep), and the admin-API document still lists it as open question 4. Whatever it becomes must not be tagged `ready`: a third party's outage must not take this portal out of rotation. |
| FR-18 | Self-awareness / base URL | ✅ | `PublicIris` carries a configured `publicBaseUrl` (AP7); stored identities are host-free and rendered under it at read time. **No default, by design** (2026-08-20): it becomes the host in every published `about`, so the component refuses to activate without it — including on an uninterpolated `$[env:...]` placeholder, which is what Felix leaves behind when `PUBLIC_BASE_URL` is unset. All ten collection resources require the service, so a misconfigured portal answers 404 instead of publishing unrebased `http://dcat.atlas/…` identities, and the `public-iris` readiness check names the setting. Remaining gap: the `Location` header on a create still comes from the request `UriInfo`, not `publicBaseUrl`, so behind a reverse proxy it can disagree with the `about` in the same response. |
| FR-4b | Reference round-trip (limitation) | ⚠️ | **Known limitation, documented 2026-08-20.** A resource carrying member references cannot be `PUT` back under SHACL enforcement: `<dataset href=…/>` names the target without its `rdf:type`, so validating the body as a standalone graph fails `sh:ClassConstraintComponent`. Intrinsic to how the model encodes a reference, not a defect in the request. Workaround — replace without members, then re-link — is in the user guide under *Modifying a resource that has members*. Invisible to the integration suite, which runs with no shapes configured. |
