...
@JakartarsResource
@JakartarsName("DataServiceReadOnlyResource")
@Component(name = "DataServiceReadOnlyResource", service = DataServiceReadOnlyResource.class, scope = ServiceScope.PROTOTYPE)
@Path("/data-services")
public class DataServiceReadOnlyResource {
	...
	public Response listDataServices(@QueryParam(Pagination.PARAM_AFTER) String after,
		...
		Page<DataService> page = dataServiceReadOnlyService.listDataServices(request);
		// GenericEntity preserves List<DataService> so the RDF body writers see the element type.
		Object entity = new GenericEntity<List<DataService>>(page.items()) {
		};
		return Pagination.respond(page, entity, request, identityRendering, DcatIds.DATA_SERVICES,
