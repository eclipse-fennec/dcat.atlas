...
/**
 * Test helper mirroring the impl bundle's {@code TestResourceSets}: a stand-alone
 * {@link ResourceSet} that knows every DCAT-AP package and routes {@code .rdf} to
 * the RDF resource factory (the extension {@code EObjectRDFModelBuilder} relies on).
 */
public final class ValidationTestResourceSets {

	private ValidationTestResourceSets() {
	}

	public static ResourceSetFactory factory() {
		return ValidationTestResourceSets::newResourceSet;
	}
	...
	public static ResourceSet newResourceSet() {
		...
		resourceSet.getResourceFactoryRegistry().getExtensionToFactoryMap().put("rdf", new RdfResourceFactoryImpl());
		return resourceSet;
	}
}
